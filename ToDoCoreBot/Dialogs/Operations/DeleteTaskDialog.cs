...
namespace ToDoCoreBot.Dialogs.Operations
{
    public class DeleteTaskDialog : ComponentDialog
    {
        ...
        private async Task<DialogTurnResult> ShowTasksStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            await stepContext.Context.SendActivityAsync(MessageFactory.Text("Please select the task you wat to delete."), cancellationToken);
        ...
        private async Task<DialogTurnResult> DeleteTasksStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            {
                await stepContext.Context.SendActivityAsync(MessageFactory.Text("Task '" + taskToDelete + 
                    "' could not be deleted. Either it has been already deleted or some error occurred."), cancellationToken);
                return await stepContext.EndDialogAsync(null, cancellationToken);
            }
        }
