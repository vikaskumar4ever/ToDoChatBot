...
using ToDoCoreBot.Dialogs.Operations;
using ToDoCoreBot.Utilities;

namespace ToDoCoreBot.Dialogs
{
    public class MainDialog : ComponentDialog
    {
        ...
        private async Task<bool> UserValidation(PromptValidatorContext<string> promptContext, CancellationToken cancellationToken)
        {
            ...
            await promptContext.Context.SendActivityAsync("Please wait, while we validate your details...", cancellationToken: cancellationToken);
            if (await _cosmosDBClient.CheckNewUserIdAsync(userId,
                Configuration["CosmosEndPointURI"],
                Configuration["CosmosPrimaryKey"],
                Configuration["CosmosDatabaseId"],
                Configuration["CosmosContainerId"],
                Configuration["CosmosPartitionKey"]))
            {
                await promptContext.Context.SendActivityAsync("Your details is verified", cancellationToken: cancellationToken);
                User.UserID = userId;
                return true;
            }
            await promptContext.Context.SendActivityAsync("The user id you entered is not found, Please enter the correct id");
            return false;
        }
        ...
        private async Task<DialogTurnResult> UserIDStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            if (User.UserID == null)
            {
                ...
                if ("Returning User".Equals(userType))
                {
                    ...
                    {
                        Prompt = MessageFactory.Text("Please Enter UserId.")
                    }, cancellationToken);
                }
                else
                {
                    ...
                    } while (await _cosmosDBClient.CheckNewUserIdAsync(userId,
                    Configuration["CosmosEndPointURI"],
                    Configuration["CosmosPrimaryKey"],
                    Configuration["CosmosDatabaseId"],
                    Configuration["CosmosContainerId"],
        ...
        {
            stepContext.Values["Operation"] = ((FoundChoice)
